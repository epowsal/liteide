...
//WINPTY_API winpty_config_t *(*winpty_config_new)(UINT64 agentFlags, winpty_error_ptr_t *err /*OPTIONAL*/);

/* Free the cfg object after passing it to winpty_open. */
//WINPTY_API void (*winpty_config_free)(winpty_config_t *cfg);

//WINPTY_API void(*winpty_config_set_initial_size)(winpty_config_t *cfg, int cols, int rows);

/* Set the mouse mode to one of the WINPTY_MOUSE_MODE_xxx constants. */
//WINPTY_API void(*winpty_config_set_mouse_mode)(winpty_config_t *cfg, int mouseMode);
...
//WINPTY_API void(*winpty_config_set_agent_timeout)(winpty_config_t *cfg, DWORD timeoutMs);



//WINPTY_API winpty_t *(*winpty_open)(const winpty_config_t *cfg, winpty_error_ptr_t *err /*OPTIONAL*/);
...
//WINPTY_API winpty_spawn_config_t *(*winpty_spawn_config_new)(UINT64 spawnFlags,LPCWSTR appname ,LPCWSTR cmdline ,LPCWSTR cwd ,LPCWSTR env ,winpty_error_ptr_t *err /*OPTIONAL*/);
//WINPTY_API void (*winpty_spawn_config_free)(winpty_spawn_config_t *cfg);
//WINPTY_API BOOL(*winpty_spawn)(winpty_t *wp,const winpty_spawn_config_t *cfg,HANDLE *process_handle ,HANDLE *thread_handle ,DWORD *create_process_error ,npty_error_ptr_t *err );
//WINPTY_API BOOL(*winpty_set_size)(winpty_t *wp, int cols, int rows,winpty_error_ptr_t *err);
//WINPTY_API int(*winpty_get_console_process_list)(winpty_t *wp, int *processList, const int processCount,winpty_error_ptr_t *err );
...
{
    return "";//return QString::fromStdWString(winpty_error_msg(error_ptr));
}
